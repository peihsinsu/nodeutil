...
function initial(){
   ...
   */
  smtpTransport = nodemailer.createTransport(smtpOption);
}
...
function sendMail(sender, receivers, subject, msg, isClose, callback) {
  ...
  opt.html = msg;
  log.trace('[INFO]Using smtpOption:');
  log.trace(smtpOption);
  log.trace('[INFO]Using mailOptions:');
  log.trace(opt);
  smtpTransport.sendMail(opt, function(error, response){
    if(error){
       log.error('[ERROR]%s',JSON.stringify(error));
    } else {
       log.trace("Message sent: " + response.message);
    }
    if(isClose)
